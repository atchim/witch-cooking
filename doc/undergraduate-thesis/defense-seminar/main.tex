...
\begin{document}
  ...
  \begin{frame}{A Sincronização de Nós}
    \begin{itemize}
      \item Durante a etapa de teste do funcionamento do \witchcooking,
            notou-se uma peculiaridade --- não documentada
            \cite{tree-sitter-2023-node} --- do \treesitter
            \begin{itemize}
              \item Ao sincronizar um nó, as edições ocorridas tangentes à sua
                    extensão não são contabilizadas
                    \begin{itemize}
                      \item Resultando em um potencial para erros de formatação
                    \end{itemize}
            \end{itemize}
    \end{itemize}
  \end{frame}
  ...
  \begin{frame}[fragile]{A Sincronização de Nós}
    \begin{minipage}[t]{\halfpart}
      \begin{codesnippet}[rust]{Funções Aninhadas em Rust}
        fn foo() {fn bar() {baz()}}
      \end{codesnippet}
    \end{minipage}
    \hfill
  ...
  \begin{frame}[fragile]{Contribuições}
    \begin{itemize}
      ...
      \item Mais precisamente
            \begin{itemize}
              \item O predicado \codesnippetinline{space!} opera similarmente
                    ao \textit{combinator}
                    \codesnippetinline{|$\leftrightarrow$|}[escapeinside=||]
              \item O predicado \codesnippetinline{indent!} funciona análogo ao
                    uso de
  ...
  \begin{frame}[fragile]{Trabalhos Futuros}
    \begin{itemize}
      \item Desenvolver um algoritmo dedicado à sincronização de nós, capaz de
            sincronizar corretamente qualquer nó, mesmo após ele ter sofrido
            edições tangentes à sua extensão
      \item Aprimorar o predicado \codesnippetinline{indent!}
            \begin{itemize}
              \item Melhorar a formatação de nós com regras de indentação
                    relativa positiva, de modo que a indentação siga o
                    deslocamento do nó pai, em vez de se basear no primeiro
                    caractere não espaço encontrado na linha do pai
              \item Seria mais coerente que esse predicado indentasse o bloco
                    inteiro ao qual o nó se estende, em vez de se limitar à
                    linha inicial do nó em questão
            \end{itemize}
    \end{itemize}
  \end{frame}
  ...
  \begin{frame}[fragile]{Trabalhos Futuros}
    \begin{itemize}
      \item Aprimoramentos relacionados ao dinamismo da formatação
            \begin{itemize}
              \item A formatação com base no \textit{CPL}
              \item A otimização para determinar o melhor layout
            \end{itemize}
      \item Disponibilizar diretrizes de formatação que sigam os estilos de
            formatação convencionais para as linguagens de programação mais
            populares (a princípio)
    \end{itemize}
  \end{frame}
