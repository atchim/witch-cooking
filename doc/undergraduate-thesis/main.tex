...
\begin{document}
  ...
  \begin{inparaenum}
    ...
    \item \label{item:loop} um segundo \textit{loop} percorre as coleções de
          nós capturados para cada correspondência do padrão em questão. No
          ...
          \node (fmt) [below=of init] {
            \begin
              ...
              \node
                (VII)
                [below=of VI]
                {Interrompe o \cref{item:loop} se o padrão é enraizado};
              \draw [->] (I) -- (II);
              \draw [->] (II) -- (III);
              \draw [->] (III) -- (IV);
              \draw [->] (IV) -- (V);
