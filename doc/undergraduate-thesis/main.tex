...
\begin{document}
  ...
  \begin{lrbox}{\mexpressionexample}
    \codesnippetinline[fontsize=\footnotesize]{car[cons[A; B]]}
  \end{lrbox}

  \newsavebox\sexpressionexample
  \begin{lrbox}{\sexpressionexample}
    \codesnippetinline[fontsize=\footnotesize][lisp]{(car (cons A B))}
  \end{lrbox}
  ...
  \treesitter. Como abordado no \autoref{chapter:tree-sitter}, essa linguagem
  foi originalmente concebida para buscar padrões na árvore sintática via
  \begin{inparaenum}
  ...
  \ref{snippet:add-two-rust}, escritos nas linguagens de programação C e Rust,
  respectivamente. Embora ambos os trechos desempenhem papéis idênticos --
  i.e., definem uma função que retorna a soma do parâmetro \textit{x} com 2 --,
  suas respectivas árvores de sintaxe analisadas -- apresentadas nos trechos de
  código \ref{snippet:add-two-c-tree} e \ref{snippet:add-two-rust-tree} --
  exibem diferenças significativas. Dessa forma, para buscar a correspondência
  do trecho em que o parâmetro \textit{x} é definido em ambos os códigos
  \ref{snippet:add-two-c} e \ref{snippet:add-two-rust}, é necessário distintas
  consultas, apresentadas pelos trechos de código \ref{snippet:add-two-query-c}
  e \ref{snippet:add-two-query-rust}, respectivamente.
  ...
  \begin{minipage}[t]{.49\textwidth}
    ...
    \begin
      ...
      [showspaces,fontsize=\ABNTEXfontereduzida]%
    ...
    \captionof*
      ...
      { \ABNTEXfontereduzida
        Fonte autoral.
        Nós anônimos foram omitidos.
        Quebra de linha presente.
        Espaços estão representados pelo caractere `\textvisiblespace'.}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{.49\textwidth}
    \begin
  ...
  \begin{minipage}[t]{.51\textwidth}
    \begin
      {codesnippet}*%
      [scheme]%
      {Consulta para função \textit{add\_two} em C}%
      []%
      [label={snippet:add-two-query-c}]
      (parameter_declaration
        type: (primitive_type)
        declarator: (identifier))
    \end{codesnippet}
  ...
  \begin{minipage}[t]{.47\textwidth}
    \begin
      {codesnippet}*%
      [scheme]%
      {Consulta para função \textit{add\_two} em Rust}%
      []%
      [label={snippet:add-two-query-rust}]
      (parameter
        pattern: (identifier)
        type: (primitive_type))
    \end{codesnippet}
