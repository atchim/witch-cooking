...
\begin{document}
  ...
  Uma das operações fundamentais da formatação consiste na inserção de
  espaçamento entre \textit{\gloss{token}s}. Nesse contexto, o \witchcooking
  disponibiliza o predicado \codesnippetinline{space!}. Esse predicado funciona da
  seguinte forma \codesnippetinline{(#space! [sep [lower [upper]]] a b)}, onde
  \begin{inparaenum}
    ...
    \item \codesnippetinline{lower} e \codesnippetinline{upper} são inteiros
          não negativos que definem o intervalo inclusivo para a não-operação
          do espaçamento, sendo que
          \begin{inparaenum}
            \item se \codesnippetinline{upper} não for especificado, o
                  intervalo resultante será
                  $\left[1, \codesnippetinline{lower}\right]$,
            \item senão, o intervalo será
                  $ \left
                      [ \codesnippetinline{lower},
                        \codesnippetinline{upper}\right]$. Além disso,
  ...
  \begin{minipage}[t]{\textwidth}
    \begin
      {codesnippet}%
      [scheme]%
      {Consulta Para Separar Itens de uma Lista em Rust}%
      {item-sep}
      (array_expression
        "," @comma
        . (_) @item
