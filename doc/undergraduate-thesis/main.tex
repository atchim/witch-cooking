...
\begin{document}
    ...
    \item LISP apresentava uma sintaxe distinta e expressiva, baseada em listas
          ...
          \footnote
            {%
              ...
              \textit{M-expression} \boxed{\texttt{car[cons[A; B]]}} seria
              equivalente à \textit{S-expression}
              \boxed{\texttt{(car (cons A B))}}. No entanto, assim que a
              linguagem LISP foi implementada, os programadores prontamente
              adotaram o uso das \textit{S-expressions} em vez das
              \textit{M-expressions}, tornando as \textit{S-expressions} a
              forma predominante de representar a estrutura do código na
              linguagem.}%
  ...
  específicos, como \boxed{\texttt{.vhs}} e \boxed{\texttt{.rasi}} --, os quais
  têm parsers gerados pelo \treesitter e, consequentemente, são suportados pelo
  sistema.
  ...
  natureza prototípica, para a formatação de código-fonte. Para essa
  finalidade, a ferramenta requer a submissão do código-fonte a ser formatado,
  juntamente com o arquivo contendo as instruções de formatação. Em alinhamento
  com sua natureza experimental, o \witchcooking proporciona o código-fonte
  formatado resultante como saída, sem modificar o conteúdo original,
  efetivamente estabelecendo uma formatação segura.
  ...
  \begin{inparaenum}
    ...
    \item \boxed{\texttt{\textbf{-q} \textit{QUERY}}} é utilizado para definir
          o caminho para o arquivo -- escrito na linguagem de consulta do
          \treesitter{} -- no qual as instruções de formatação estão definidas,
          e
  ...
  Considere, por exemplo, os trechos de código \ref{snippet:add-two-c} e
  \ref{snippet:add-two-rust}, os quais apresentam uma função que retorna a soma
  do parâmetro com 2.
  ...
  \begin{minipage}[t]{.49\textwidth}
    \begin
      {codesnippet}%
      {Função \texttt{``add\_two''} em C}%
      [label={snippet:add-two-c}]
      int add_two(int x) {
        return x + 2;
      }
    \end{codesnippet}
    ...
    \begin{codesnippet}*%
      [text]%
      {Árvore Sintática da Função \texttt{``add\_two''} em C}%
      [label={snippet:add-two-c-tree}]
      function_definition
        type: primitive_type
  ...
  \begin{minipage}[t]{.49\textwidth}
    \begin
      {codesnippet}%
      [rust]%
      {Função \texttt{``add\_two''} em Rust}%
      [label={snippet:add-two-rust}]
    ...
    \begin{codesnippet}*%
      [text]%
      {Árvore Sintática da Função \texttt{``add\_two''} em Rust}%
      [label={snippet:add-two-rust-tree}]
      function_item
        name: identifier
