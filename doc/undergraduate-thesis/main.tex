...
\begin{document}
  ...
  \codesnippetinline{-q QUERY}. Para realizar um procedimento de efetivo de
  formatação, é necessário
  \begin{inparaenum}
    \item definir um padrão de correspondência onde o procedimento atuará;
    \item capturar nós que
          \begin{inparaenum}
            \item são alvos do procedimento (i.e., serão formatados) ou que
            \item auxiliarão nas operações feitas;
          \end{inparaenum}
    \item opcionalmente, aplicar configurações via o predicado \textit{set!};
          e, por fim,
