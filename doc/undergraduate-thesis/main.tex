...
\begin{document}
  ...
  \begin{inparaenum}
    \item na linha 6, a propriedade \codesnippetinline{indent-rule} é
          ...
          vigente em adição ao deslocamento dos nós pais. Além disso,
            ...
            \item se o valor for \codesnippetinline{"+1"}%
                  \footnote
                    { É importante ressaltar que, ao fornecer um argumento para
                      ...
                      os delimitadores. Consequentemente, é possível
                      especificar:
                      \begin{inparaenum}[(I)]
                        \item uma regra de indentação absoluta via
                              \usebox{\indentruleabs},
                        \item uma regra de indentação relativa e negativa via
                              \usebox{\indentruleneg}, e
                        \item uma regra de indentação relativa e positiva via
                              \usebox{\indentrulepos}.
                      \end{inparaenum}}%
                  , os nós capturados
  ...
  Além disso, ao final do desenvolvimento do \witchcooking, foi descoberto um
  problema na formatação que ocorre sob condições específicas. Para compreender
  a complexidade envolvida nesse problema, é essencial ter uma compreensão do
  funcionamento do processo de formatação; mais especificamente, como o
  processo de edição do código em formatação opera. Conforme mencionado na
  \cref{section:fmt-algorithm}, o algoritmo de formatação é composto, entre
  outros componentes, por uma estrutura editora. Essa estrutura retém o
  código-fonte em formatação e registra as edições (ou modificações) realizadas
  pelos predicados.
