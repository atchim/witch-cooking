...
\begin{document}
  ...
  Nessa fase, a dinâmica se baseia em um \textit{\gloss{loop}} que
  \begin{inparaenum}
    \item itera sobre todos os padrões especificados no arquivo de instruções
          de formatação. Em cada iteração, inicialmente,
    ...
    \item um segundo \textit{loop} percorre as coleções de nós capturados para
          cada correspondência do padrão em questão. No corpo dessa estrutura
          de repetição,
    \item são aplicadas as configurações especificadas no padrão via o
          predicado \textit{builtin} \textit{set!}.
  \end{inparaenum}
      ...
      { \begin
          ...
          \node (fmt) [below=of init] {
            \begin
              ...
              [ every node/.style=
                  {align=center,draw,inner sep=.5em,text width=16em}]
              \node [draw=none] (label) {Formatação};
              \node (I) [below=of label] {Itera sobre as correspondências};
