...
\begin{document}
  ...
  as possiblidades de customização do usuário. Finalmente, é também comum que
  essas ferramentas de formatação não permitam que o usuário defina regras de
  ...
  dos \textit{\gloss{opinionated} softwares} \cite{eccles-2015-rise}, também
  conhecidos como softwares opinados. Esses softwares preestabelecem práticas
  que devem ser seguidas rigorosamente, visando deliberadamente frustrar ou
  dificultar práticas destoantes \cite{lance-2021-about}. Os
  \textit{prettyprinters} primitivos, tais como o \textit{SOAP}
  \cite{scowen-1971-soap}, eram naturalmente opinativos devido à simplicidade e
  às limitações dos algoritmos de formatação existentes. Conforme mencionado
  anteriormente, à medida que os algoritmos de formatação de código-fonte eram
  aprimorados, tornando-se mais robustos e sofisticados, os
  \textit{prettyprinters} passaram a proporcionar mais customizações aos
  usuários. No entanto, atualmente, a abordagem opinativa tem se tornado
  relevante também nos softwares de formatação de código-fonte
  \cites[5--6,8]{griesemer-2022-cultural}.
