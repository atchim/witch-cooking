...
\begin{document}
  ...
  Considere, por exemplo, os trechos de código \ref{snippet:hello-c} e
  \ref{snippet:hello-rust}, os quais

  \noindent
  \begin{minipage}{.5\textwidth}
    \begin
      {codesnippet}%
      {\textit{``Hello, World!''} em C}%
      [label={snippet:hello-c}]
      #include <stdio.h>
      int main() {
        printf("Hello, world!\n");
        return 0;
  ...
  \begin{minipage}{.5\textwidth}
    \begin
      {codesnippet}%
      [rust]%
      {\textit{``Hello, World!''} em Rust}%
      [label={snippet:hello-rust}]
      fn main() {
        println!("Hello, world!");
      }
    \end{codesnippet}
