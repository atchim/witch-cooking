...
\begin{document}
  ...
  Contudo, na \cref{section:missing}, a partir de um cenário mais complexo de
  ...
  Adicionalmente, ainda na \cref{section:missing}, a partir da exemplificação
  do predicado \codesnippetinline{indent!}, foram discutidos:
  \begin{inparaenum}
    \item as limitações da implementação atual desse predicado ao aplicar
          diferentes regras de indentação, além de
  ...
  continua a ser diretamente influenciado por esse parâmetro. No entanto,
  apesar dessa importância que o \textit{CPL} normalmente desempenha na
  formatação, vale ressaltar que o \witchcooking atualmente não fornece algum
  mecanismo de formatação conforme esse aspecto. Mais precisamente, a
  ferramenta carece de mecanismos que proporcionem formatação condicionada, em
  geral.
