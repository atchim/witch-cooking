\documentclass[11pt, openright, twoside, a4paper, english, brazil]{abntex2}
...
% Cover & Title Page Info {{{2
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\titulo
  { Witch Cooking: Formatação de Código via Linguagem de Consulta do
    Tree-Sitter}
...
% ABNT's NBR 14724 Metrics Compliance {{{3
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\renewcommand
  {\imprimircapa}
  { \begin{capa}
      \center\ABNTEXchapterfont\bfseries\Large
      \includegraphics[width=\textwidth]{ifba-irece-logo.png}
      \par\vfill\imprimirautor
      \par\vfill\imprimirtitulo\vfill\vfill
      \par\large\imprimirlocal\break\imprimirdata
    \end{capa}}
...
\renewcommand
  ...
  { \begin{center}
      ...
      \imprimirautor
      \par\vfill\vfill\imprimirtitulo
      \vfill
      { \normalsize
        \mdseries
        \hspace{.45\textwidth}
        ...
        \end{minipage}}
      \vfill\vfill
      \large\imprimirlocal\break\imprimirdata
    \end{center}}
\makeatother
...
\setglosslabel{\sffamily\bfseries#2\ifglossshort{ (#3)}{}}
\renewenvironment{thegloss}
  { \chapter*{\glossname}
    \addcontentsline
      {toc}
      {chapter}
      {\texorpdfstring{\MakeTextUppercase{\glossname}}{\glossname}}}
  {\stopglosslist}
\makegloss
% }}}3
...
\begin{resumo}
	...
	automatizar a formatação de código. No entanto, essas ferramentas apresentam
	limitações no número de linguagens suportadas, nas opções de configuração e
	não permitem a definição de regras de formatação personalizadas. Visando
	superar ou reduzir as limitações frequentemente encontradas nas ferramentas
	convencionais de formatação, neste trabalho é apresentado o
	\textit{Witch Cooking}, um software protótipo que tem como objetivos:
	\begin{inparaenum}
...
com aspectos desejados, como a legibilidade, que é fundamental no ciclo de vida
do software \cites{buse-2009-learning}{oliveira-2020-evaluating}.
...
A formatação do código pode ser realizada manualmente pelo programador, embora
...
programação. Além disso, algumas delas são \textit{\gloss{opinionated}}, assim
restringindo as possiblidades de customização do usuário. Finalmente, é também
comum que essas ferramentas de formatação não permitam que o usuário defina
regras de formatação personalizadas.

Em ambientes de desenvolvimento modernos, além da utilização de ferramentas que
automatizam a formatação do código, é também comum a utilização do
...
primeiramente em 2019, tendo sido inicialmente desenvolvido por Max Brunsfeld.
Desde seu lançamento, o \textit{Tree-Sitter} tem ganhado popularidade na
...
que pode ser utilizada para gerar \textit{\gloss{parser}s} para uma linguagem a
...
meio de \textit{\gloss[Word]{sexp}} e buscar correspondências. A linguagem de
