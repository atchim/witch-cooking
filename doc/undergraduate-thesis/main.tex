...
\begin{document}
  ...
  \begin{resumo}
    ...
    formatação, neste trabalho é apresentado o \textit{Witch Cooking}, um
    software prototípico que tem como objetivos
    \begin{inparaenum}
  ...
  \begin{inparaenum}
    ...
    \item permitir que o usuário possa optar por estilos de formatação
          ...
          relação à estruturação do código-fonte. Assim, para atender o
  ...
  padrões por meio de \textit{\gloss[Word]{sexp}s} e buscar correspondências. A
  ...
  \begin{inparaenum}
    ...
    \item captura de nós correspondentes a construções gramaticais
          (e.g., funções, classes, métodos, etc.), os quais são
          frequentemente utilizados em rotinas de programação tais como a
          remoção e navegação do código.
  \end{inparaenum}

  Em resumo, o sistema \textit{Tree-Sitter} se mostra uma solução inovadora e
  ...
  declarativa oferecida pelo \textit{Tree-Sitter} se mostra uma importante
  ...
  formatado, juntamente com o arquivo contendo as instruções de formatação. Em
  ...
  conteúdo original, efetivamente estabelecendo uma formatação segura.
  ...
  \begin{inparaenum}
    ...
    \item \boxed{\texttt{\textbf{-q} \textit{QUERY}}} é utilizado para
          definir o caminho para o arquivo -- escrito na linguagem de consulta
          do \textit{Tree-Sitter} -- no qual as instruções de formatação estão
          definidas, e
  ...
  Como mencionado na seção anterior, o \textit{Witch Cooking} formata o
  ...
  consulta do \textit{Tree-Sitter}. Essas instruções constituem-se da
  \begin{inparaenum}
    \item busca de padrões -- i.e., \textit{pattern matching} -- em árvores
          sintáticas, da
    \item configuração, e da
    \item aplicação de predicados.
  \end{inparaenum}
  No entanto, como abordado no \autoref{chapter:tree-sitter}, a linguagem de
  consulta do \textit{Tree-Sitter} concebia originalmente a busca de padrões em
  árvores sintáticas. Portanto, para viabilizar a formatação de código-fonte, o
  \textit{Witch Cooking} introduz um conjunto adicional de configurações e
  predicados.
