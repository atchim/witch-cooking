...
\titulo{\worktitle: \worksubtitle}
\autor{Átila Dias da Gama e Silva}
\local{Irecê}
\data{\the\year}
\orientador{Rafael Xavier de Oliveira Souza}
\instituicao{\ifba}
...
\begin{document}
  ...
  código seja padronizado e consistente. Embora a ideia de desenvolver um
  formatador multilíngue de código-fonte não seja novidade --- e.g., o
  algoritmo apresentado por \textcite{oppen-1980-prettyprinting} formata
  independentemente da linguagem ---, essas ferramentas são normalmente
  restritas a uma linguagem específica ou a uma família de linguagens de
  programação --- e.g., \textit{Black} \cite{black-2023-black} e \textit{rfmt}
  \cite{yelland-2015-rfmt}. Além disso, essas ferramentas costumam oferecer uma
  quantidade limitada de configurações de estilização, dificultando a
  ...
  \begin{inparaenum}
    ...
    \item definir configurações de estilização para o predicado
          \codesnippetinline{set!}, nativo da linguagem de consulta do sistema;
          e
  ...
  \apud{harris-1956-keyboard}[1]{yelland-2015-new}. Esses métodos ganharam
  ...
  A operação do algoritmo de formatação --- cujo pseudocódigo é exemplificado
  no \cref{snippet:python:pseudo} --- divide-se nas etapas de:
  \begin{inparaenum}
    \item inicialização,
    \item formatação, e
    \item retorno%
  ...
  \begin{minipage}[t]{\textwidth}
    \begin
      {codesnippet}%
      [python]%
      {Pseudocódigo do Algortimo de Formatação}%
      {pseudo}%
      [fontsize=\scriptsize]
      def cook(
        parser: Parser,
        ...
        query = Query.new(language, query_src)
        editor = Editor.new(src_code)
        settings = Settings.default()
        matches = Matches.new(query_cursor.matches(query, tree.root_node(), src_code))
        for pattern_index, array_of_capture_index_to_nodes in matches:
          is_pattern_rooted = query.is_pattern_rooted(pattern_index)
          scope = Scope.LOCAL if is_pattern_rooted else Scope.GLOBAL
          nodes_providers = map(
            lambda capture_index_to_nodes: NodesProvider.new(
              capture_index_to_nodes, matches.id_to_node()
            ),
            array_of_capture_index_to_nodes,
          )
          for nodes_provider in nodes_providers:
            for query_property in query.property_settings(pattern_index):
              setting_parsers.parse(query_property, scope, nodes_provider, settings)
            for query_predicate in query.general_predicates(pattern_index):
              predicates.parse(
                query, query_predicate, scope, nodes_provider, settings, editor,
              )
            settings.reset()
            if not is_pattern_rooted: break
        return str(editor)
    \end{codesnippet}
    \vspace{-3ex}
  ...
  \begin{inparaenum}
    ...
    \item \codesnippetinline{close} tem sua regra de indentação definida como o
          ...
          \codesnippetinline{indent-offset!}. Concluindo a explicação da
          consulta, ao final do segundo padrão, é utilizado o predicado
          \codesnippetinline{indent!}, passando as respectivas capturas
          \codesnippetinline{item} e \codesnippetinline{close} como argumentos.
          O resultado da aplicação dessa consulta está disponível para
          verificação no \cref{snippet:rust:1tbs-foo}.
  \end{inparaenum}
  ...
  Contudo, é importante ressaltar que o uso dos predicados é de total
  responsabilidade do usuário. Por exemplo, considere a lista presente no
  seguinte trecho de código em linha, escrito em Python:
  \codesnippetinline[python]{[False, 0, 'a']}. Nesse cenário, é possível
  utilizar o predicado \codesnippetinline{space!} para erroneamente espaçar os
  itens dessa lista com um separador caracterizado pelo caractere `x', como
  ilustrado a seguir:
  \codesnippetinline
    [scheme]
    {(list (_) @item . (_) @next (#space! x @item @next))}%
  . Ao aplicar a consulta, o código-fonte resultante da formatação apresenta o
  seguinte conteúdo: \codesnippetinline[python]{[Falsex0x'a']}. Esse código
  formatado não é apenas desfigurado, mas também sintaticamente inválido.

  Outro aspecto relevante a ser notado é que diferentes linguagens de
  programação exigem desafios distintos na formatação de código-fonte. Por
  exemplo, enquanto há linguagens que proporcionam uma estruturação de código
  bastante flexível --- e.g., Rust, LISP, etc. ---, outras impõem restrições
  mais severas quanto à organização do código. Consequentemente, a complexidade
  na elaboração das consultas está atrelada ao intrincamento da sintaxe da
